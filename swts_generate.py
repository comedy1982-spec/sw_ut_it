...
def _emit_harness(spec: TestSpec) -> str:
    ...
    argv[1] 이 있으면 해당 케이스 ID 하나만 실행 (TC별 커버리지 측정용)."""
    L = []
    ...
    L.append('  const char* _tc_filter = (argc > 1) ? argv[1] : "";')
    for case in spec.cases:
        if case.get("verdict") == "manual":
            continue
        cid = case["id"]
        L.append(f'  if (_tc_filter[0] == 0 || strcmp(_tc_filter, "{cid}") == 0) {{')
        L.append(f"    /* {cid} */")
...
# 5. 계약 JSON 합성
# ============================================================
def per_tc_coverage(spec: TestSpec, exe: str, workdir: str) -> dict[str, dict[int, int]]:
    """각 TC를 단독 실행하여 TC별 라인 히트맵을 반환."""
    base = os.path.basename(spec.src_file)
    stem = os.path.splitext(base)[0]
    tc_hits: dict[str, dict[int, int]] = {}
    for case in spec.cases:
        cid = case["id"]
        if case.get("verdict") == "manual":
            tc_hits[cid] = {}
            continue
        # gcda 초기화 (이전 TC 누적 제거)
        for fname in os.listdir(workdir):
            if fname.endswith(".gcda") and stem in fname:
                try:
                    os.remove(os.path.join(workdir, fname))
                except OSError:
                    pass
        subprocess.run([exe, cid], cwd=workdir, capture_output=True, text=True, timeout=10)
        tc_hits[cid] = coverage_lines(spec, workdir)
    return tc_hits


def build_source(spec: TestSpec, hits: dict[int, int],
                 tc_hits: Optional[dict[str, dict[int, int]]] = None) -> list[dict]:
    ...
    for i, text in enumerate(lines, 1):
        ...
        tcs = [tc_id for tc_id, th in tc_hits.items() if th.get(i, 0) > 0] if tc_hits else []
        out.append({"n": i, "text": text.rstrip("\n"), "cov": cov,
                    "hits": max(h, 0), "tcs": tcs})
    return out
...
def generate(spec: TestSpec, log=lambda lvl, t: None) -> dict:
    ...
    try:
        ...
        log("info", "[Phase1] TC별 개별 커버리지 측정")
        tc_hits = per_tc_coverage(spec, exe, workdir)
        source = build_source(spec, hits, tc_hits)
